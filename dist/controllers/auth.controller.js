...
export const handleGitCallback = async (req, res) => {
    const { code, error } = req.query;
    // exchange code for token and fetch user details
    const githubUserData = await handleAccessTokenAndUserFetch(code);
...
export const gitLogout = (req, res) => {
...
const handleAccessTokenAndUserFetch = async (code) => {
    ...
    if (!accessToken)
        throw new BadRequestError("Failed to retrive access token");
